...
import numpy as np
import os
from train_CBMA import *


# 这里可以继续定义动态填充、加载模型和进行预测的逻辑...
class DynamicPad:
    """保持长宽比的动态填充"""

    def __init__(self, max_dim=224, fill=0):
...
if __name__ == "__main__":
    ...
    # 加载模型
    models_paths = ['../model/best_model_se_face4_19_5826.pth', '../model/best_model_se_face5_19_59.pth', '../model/best_model_se_face6_19_3552.pth']
    models = []
    ...
    with open('../annotations/val.txt', 'r') as f:
        for line in tqdm(f, desc='Processing images'):
            img_name_orig, true_age = line.strip().split('\t')
            true_age = float(true_age)
            img_name = img_name_orig.replace('A*', 'A_')

            # 加载和预处理图像
            img_path = os.path.join('../dog_face_val', img_name)
            try:
                ...
                with torch.no_grad():
                    ...
                    # 计算所有预测结果的平均值
                    final_pred_age = sum(all_preds) / len(all_preds)

                    new_line = f"{img_name_orig}\t{final_pred_age}\n"
