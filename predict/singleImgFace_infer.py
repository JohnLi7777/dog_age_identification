...
if __name__ == "__main__":
    ...
    # 加载模型
    model = torch.load('../model/best_model_se_face4_19_5826.pth', map_location=device)
    ...
    for result in results:  # results 是一个列表，每个元素是一个检测结果
        ...
        if boxes is not None:
            for i in range(len(boxes.xyxy)):
                ...
                # 数据预处理
                tensor_img = val_transform(dog_face_pil).unsqueeze(0).to(device)
                # 执行预测
                with torch.no_grad():
